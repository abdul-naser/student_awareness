...
if ($distinctWilayatsQuery) {
    while ($row = $distinctWilayatsQuery->fetch_assoc()) {
        ...
        $countQuery = $mysqli->query("SELECT COUNT(*) AS count FROM awareness_programme WHERE wilayat = '$wilayat' AND YEAR(date) = '$input_1'");
        if ($countQuery) {
    ...
    // Assuming you have an array named $wilayatCounts in your PHP code
     ...
     myChart2S = new Chart(ctx2S, {
        ...
        data: {
            ...
            datasets: [{
                label: "البرامج في كل ولاية لعام <?php  echo $input_1 ?> ",
